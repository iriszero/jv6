...
{
  int tid = (int) arg;
	printf(1, "racingthread tid - %d called\n", tid);
  int i;
  //int j;
  int tmp;
...
racingtest(void)
{
  ...
  for (i = 0; i < NUM_THREAD; i++){
    if ((t=thread_join(threads[i], &retval)) != 0 || (int)retval != i+1){
	  printf(1, "t=%d, retval=%d\n", t, (int)retval);
      printf(1, "panic at thread_join\n");
      return -1;
    }
	printf(1, "join succeed i=%d\n", i);
  }
  printf(1,"%d\n", gcnt);
  printf(1, "end of racingtest\n");
  return 0;
}
